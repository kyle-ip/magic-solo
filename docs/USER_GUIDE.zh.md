...
- 进入 **数字对战**（挑战）/ **实体对局**（游戏助手半场）
- **打印助手**：按卡牌 **quantity** 展开张数后导出 PDF（见 [§8](#8-打印助手)）
- 可通过页眉 **挑战** 菜单或返回本套牌页打开另一套挑战
...
| `burn` | Ember Barrage（焦炎齐射） | 红色快攻 / 直伤 |
| `skies` | Azure Skies（苍穹飞攻） | 白蓝飞行 + 清除 |
| `merfolk` | Pearl Trident（珍珠三叉戟） | 蓝人鱼节奏 / 领主 |
| `akroan` | Akroan Legion（阿喀洛斯军团） | 红白士兵（先攻） |
| `nessian` | Nessian Wilds（涅西恩荒野） | 绿色野兽（延势 / 践踏） |
| `humans` | Parish Host（教区人海） | 白色人类部族 |
| `spirits` | Spectral Chorus（幽影合唱） | 白蓝精怪 / 飞行 |
| `jund` | Bloodbraid Barrens（血辫荒原） | 黑红绿中速 |

仅 **已实现** 的异能会生效（常青关键字里目前支持：敏捷、闪现、警戒、系命、灵技、飞行/种族/全体颂歌、延势/飞行阻挡、死触、先攻/连击分步伤害、践踏溢出；另有浓雾、互斗、直伤、弹回牌库、**神授（结附与宿主死亡后落地成生物）**、**勇行（含神授目标）**、血激、营队、教区/中尉、庞大化后续、**留存**、**腐食流浆启动**、**涡心鼓动同名清场**、磨牌/抽牌、返照、掘穴、银鳃额外费用、精怪闪现光环等）。挑战半场仍无完整堆叠；海岛行、线索衍生物、未知旅程「离开再回来」、完整倾曳等仍有模型限制并在牌面注明。

### 对局中

- 手牌、地、生物与挑战半场
- 挑战侧逐张揭示并施放
- 宣告攻击、指定目标（视模式而定）、结算战斗（先攻步骤 → 普通伤害步骤）
- 可开关 **教练提示** 与浮动 **战报日志**
- 胜负 **结算屏**（回合、生命、场面等；可选 AI 战报）
- 离开回套牌页，或在页眉可见时用 **挑战** 菜单打开另一套挑战

### 与官方挑战套牌的差异

| 官方 | 本站挑战 |
| --- | --- |
| 任意构筑 | 十套精选列表；仅已实现异能会生效 |
| 完整堆叠 / 优先权 | 咒语立即结算（浓雾仍可挡吐息 / 战斗伤害） |
| 全部 Theros 机制 | 常青 + 部分效果（神授结附/落地、勇行、有限庞大化） |
| 多人变体 | 仅单人 |

若要使用任意纸牌构筑，请用 [游戏助手](#4-游戏助手)。

**实现清单**（各牌组已接线 vs 仍近似）：[CHALLENGE_IMPLEMENTATION.zh.md](./CHALLENGE_IMPLEMENTATION.zh.md)。
