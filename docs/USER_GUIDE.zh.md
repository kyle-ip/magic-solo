...
三套挑战套牌简介，链接进入各自的套牌页。封面三张浮动预览牌默认呈三角叠放，可拖动摆放（刷新后恢复默认位置）。
...
仿 Arena 的单人 PvE，以**官方挑战流程 + 精选构筑**为最高保真目标；**不是**完整 Arena / 任意构筑引擎。
...
| `terror` | UB Terror | 蓝黑节奏（掘穴 / 恐怖类清除）+ Liliana |
| `burn` | Ember Barrage（焦炎齐射） | 红色快攻 / 直伤 + Chandra |
| `skies` | Azure Skies（苍穹飞攻） | 白蓝飞行 + 清除 + Jace |
| `merfolk` | Pearl Trident（珍珠三叉戟） | 蓝人鱼节奏 / 领主 + Kiora |
| `akroan` | Akroan Legion（阿喀洛斯军团） | 红白士兵（先攻）+ Elspeth |
| `nessian` | Nessian Wilds（涅西恩荒野） | 绿色野兽（延势 / 践踏）+ Nissa |
| `humans` | Parish Host（教区人海） | 白色人类部族 + Gideon |
| `spirits` | Spectral Chorus（幽影合唱） | 白蓝精怪 / 飞行 + Teferi |
| `jund` | Bloodbraid Barrens（血辫荒原） | 黑红绿中速 + Vraska |

仅 **已实现** 的异能会生效。当前机制包括：常青关键字（敏捷、闪现、警戒、系命、灵技、飞行/种族颂歌、延势、死触、先攻/连击、践踏、海岛行等）、浓雾、互斗、直伤、弹回（→挑战牌库顶）、神授/勇行、血激、营队、教区/中尉、庞大化（含 X）、留存、腐食流浆、涡心鼓动、**结界链接放逐**、**线索**、**倾曳**、**古夫 CDA**、**传奇规则**、**鹏洛客**（忠诚异能）、**有限堆叠**（挑战咒语揭示时让过 / 陵墓游灵反击 / 浓雾）等。部分牌面仍有「挑战：…」诚实注记（如无挑战手牌时的向导置底）。

### 对局中

- 手牌、地、生物、结界/神器、鹏洛客与挑战半场；点选手牌 / 战场牌可看详情（触控横屏为紧凑详情条：卡图略小、贴合文案宽度）；坟场 / **放逐** 可点开查看；牌库旁的「法力 0」是法力池（非放逐）
- 开局 **再调度**（伦敦规则：看手后再决定；再调度后仍抓 7，再置底等同次数的牌）；结束回合手牌超过 7 先弃至 7
- 挑战侧揭示 → **堆叠优先权**（仅「让过」时自动结算）→ 结算
- 宣告攻击、指定目标、战斗（先攻 → 普通伤害）；受击 / 增益 / 浓雾 / 忠诚等有短时浮动反馈（不挡点击）；消灭裂开后飞坟场，弹回飞牌库、放逐飞放逐区
- 可开关教练提示与战报日志；胜负结算屏
...
| --- | --- |
| 任意构筑 | 十套精选列表（含少量鹏洛客） |
| 完整堆叠 / 优先权 | 有限堆叠（挑战揭示窗口） |
| 全部 Theros 机制 | 常青 + 列表已接线效果 |
| 多人变体 | 仅单人 |
