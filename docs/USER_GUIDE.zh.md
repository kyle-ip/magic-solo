...
仅 **已实现** 的异能会生效（常青关键字里目前支持：敏捷、闪现、警戒、系命、灵技、飞行颂歌、延势/飞行阻挡、死触；双击为力量×2 近似；玩家侧践踏在「勇战天神」且选用冠军时，超额伤害会溢出到克赛诺斯。另有浓雾、互斗、直伤、磨牌 / 抽牌、返照、掘穴减费、攻击展示触发、有限的庞大化起动、选择类提示等）。许多印刷文本会被简化或近似处理（牌表 **简化** 徽章）。Azure Skies 使用白法术清除（可支付 W/U 法力）。
...
| 全部 Theros 机制 | 常青 + 部分效果（含有限庞大化）；寄灵等省略 |
| 多人变体 | 仅单人 |
